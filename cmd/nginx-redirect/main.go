...
func processMergeLog(logFile string, slugMap map[string]string, existingRules map[string]bool) ([]string, int) {
	...
	for _, record := range mergeLog {
		...
		// 清理 slug，移除异常的多 slug 组合（如 "text-document-plain-text-document-txt-file"）
		srcSlug = cleanSlug(srcSlug)
		dstSlug = cleanSlug(dstSlug)

		if srcSlug == "" || dstSlug == "" {
			fmt.Printf("⚠️ 警告：Slug 为空或无效，跳过。源ID: %d, 目标ID: %d\n", record.SourceID, record.TargetID)
			continue
		}
