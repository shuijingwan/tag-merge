 ...
 * 添加日志记录
 */
function add_log_record(&$merge_log, $source_lang, $source_id, $source_name, $target_lang, $target_id, $target_name, $status) {
    $merge_log[] = [
...
if (($handle = fopen($csv_file, 'r')) !== FALSE) {
    ...
    while (($data = fgetcsv($handle)) !== FALSE) {
        ...
        if ($dry_run) {
            ...
            $zh_success = true;
            // 📝 记录中文合并成功
            add_log_record($merge_log, '中', $source_zh_id, $source_zh_name, '中', $target_zh_id, $target_zh_name, 'success');
        }
        ...
        if ($zh_success && $can_merge_en) {
            ...
            if ($dry_run) {
                ...
                if (is_wp_error($result_en)) {
                    ...
                    echo "  ✅ 英文合并成功！\n";
                    // 📝 记录英文合并成功
                    add_log_record($merge_log, '英', $source_en_id, $source_zh_name, '英', $target_en_id, $target_zh_name, 'success');
                }
            }
        }
        $processed++;
...
// 📝 写入 JSON 日志文件（仅非模拟模式）
if (!$dry_run && !empty($merge_log)) {
    $log_file = __DIR__ . '/output/merge_log.json';
    $log_dir = dirname($log_file);
    if (!is_dir($log_dir)) {
        mkdir($log_dir, 0755, true);
    }
    $json_content = json_encode($merge_log, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    if (file_put_contents($log_file, $json_content)) {
        echo "📝 日志已保存至: {$log_file}\n";
        echo "💡 此文件供 Step 4 (Nginx 规则生成) 使用\n";
    } else {
        echo "❌ 日志文件写入失败\n";
    }
}
...
if ($dry_run) {
    echo "💡 提示：去掉 --dry-run 参数后才会真正写入数据库并生成日志文件。\n";
}